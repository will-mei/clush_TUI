...
class TextfieldBase(widget.Widget):
    ...
    def get_char_of_cursor(self, x=0):
        _char_index = self.cursor_position + x 
        try:
            #char_under_cur = self.value[_char_index] #use the real value
            char_under_cur = self.display_value(self.value)[_char_index]
        except IndexError:
            char_under_cur = ' '
        except TypeError:
            char_under_cur = ' '
        return char_under_cur
    ...
    #        char_under_cur = ' '
        ...
        char_under_cur = self.get_char_of_cursor()
        char_under_cur_width = char_width_tools.get_char_width(char_under_cur)
        if self.begin_at == 0:
    ...
    # get printable value to display 
    def display_value(self, value):
        if value == None:
            return ''
        else:
            try:
                #str_value = wide_str(value)
                str_value = value 
            except UnicodeEncodeError:
                #str_value = wide_str(self.safe_string(value))
                str_value = self.safe_string(value)
                return str_value
            except ReferenceError:                
                return ">*ERROR*ERROR*ERROR*<"
            #return wide_str(self.safe_string(str_value))
    ...
    def _get_string_to_print(self):
        ...
        if not string_to_print:
            return None
        # based on self.begin_at 
        string_to_print = string_to_print[self.begin_at:self.maximum_content_width + self.begin_at - self.left_margin]
        
        if sys.version_info[0] >= 3:
            string_to_print = self.display_value(self.value)[self.begin_at:self.maximum_content_width + self.begin_at - self.left_margin]
        else:
    ...
    def _print_pre_unicode(self):
        ...
        #string_to_print = wide_str(self.display_value(self.value))
        string_to_print = self.display_value(self.value)
        ...
        if self.syntax_highlighting:
            ...
            for index in range(len(string_to_print[self.begin_at:self.maximum_content_width + self.begin_at-self.left_margin])):
                try:
                    ...
                    highlight = curses.A_NORMAL
                # add _double_width_adjust 
                _char = string_to_print[self.begin_at + index]
                if self.begin_at == 0:
                    _double_width_adjust = 0
                elif char_width_tools.get_char_width(_char) == 2:
                    _double_width_adjust = 1 
                else:
                    _double_width_adjust = 0
                # print each char 
                self.parent.curses_pad.addstr(self.rely,
                                              self.relx + index + self.left_margin + _double_width_adjust, 
...
class Textfield(TextfieldBase):
    ...
    def edit(self):
        self.editing = 1
        if self.cursor_position is False:
            self.cursor_position = char_width_tools.get_str_width(self.value or '')
    ...
    def h_addch(self, inp):
        ...
        #    # add line break 
        #    _width_limitation = self.maximum_content_width -self.left_margin
        #    _breaker = ' -\n- '
        #    if ch_adding == '\n':
        #        pass
        #    elif char_width_tools.get_str_width(self.value) > _width_limitation:
        #        self.value = 
    ...
    def h_cursor_left(self, input):
        ...
        _step = char_width_tools.get_char_width(self.get_char_of_cursor(x=-1))

        #self.cursor_position -= 1
        self.cursor_position -= _step

    def h_cursor_right(self, input):
        _step = char_width_tools.get_char_width(self.get_char_of_cursor())

        #self.cursor_position += 1
        self.cursor_position += _step

    def h_delete_left(self, input):
        _step = char_width_tools.get_char_width(self.get_char_of_cursor(-1))
    ...
    def h_erase_right(self, input):
        if self.editable:
            ...
            self.cursor_position = char_width_tools.get_str_width(self.value)
            self.begin_at = 0
...
class FixedText(TextfieldBase):
    ...
    def h_cursor_right(self, input):
        ...
        #    self.begin_at += 1
        #if char_width_tools.get_str_width(self.value) - self.begin_at > self.maximum_content_width:
        if len(self.value) - self.begin_at > self.maximum_content_width:
            self.begin_at += _step
