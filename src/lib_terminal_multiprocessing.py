...
class host_con_group:
    ...
    def _group_connect(self, mode):
        pool            = ThreadPool(self.grp_info['grp_concurrency'])
        pool.map_async(
                lambda hostname: self._host_connect(hostname, mode=mode),
    ...
    # bind key of output 
    def single_run(self, cmd_list, hostname):
    ...
    def grp_run(self, cmd_list):
        ...
        pool            = ThreadPool(self.grp_info['grp_concurrency'])
        pool.map_async(
                lambda hostname: self.single_run(cmd_list, hostname),
                self._connections
        )
        pool.close()
        pool.join()
